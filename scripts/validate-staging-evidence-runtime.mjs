...
const checks = [
  ...
  ["baseline launch evaluation runs before final evidence overrides", worker.indexOf('admin.rpc("evaluate_launch_readiness")') < worker.indexOf('admin.rpc("evaluate_staging_readiness_evidence")')],
  ["final staging evidence runs before the SEO publish evaluator", worker.indexOf('admin.rpc("evaluate_staging_readiness_evidence")') < worker.indexOf('admin.rpc("evaluate_business_seo_publish_readiness")')],
  ["final readiness is read from the table after all evaluators", worker.indexOf('admin.rpc("evaluate_business_seo_publish_readiness")') < worker.indexOf("const readiness = await loadFinalReadinessSnapshot(admin") && worker.includes('from("launch_readiness_checks").select("check_key,status,required,evidence")')],
  ["final snapshot reports required-ready and blocking counts", worker.includes("required_ready: requiredReady") && worker.includes("readiness_percent:") && worker.includes("blocking_unknown:")],
