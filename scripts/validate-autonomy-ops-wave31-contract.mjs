...
const checks = [
  ...
  ["Owner runtime bootstrap accepts authenticated browser CORS preflight", bootstrap.includes('"Access-Control-Allow-Headers": "authorization, x-client-info, apikey, content-type"') && bootstrap.includes('"Access-Control-Allow-Methods": "POST, OPTIONS"') && bootstrap.includes('request.method === "OPTIONS"')],
