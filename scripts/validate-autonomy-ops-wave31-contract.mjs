...
const checks = [
  ...
  ["Runtime profiles cover Business external QA and launch", manifest.includes('"business-external-qa"') && manifest.includes('"business-launch"') && manifest.includes("NXQ_AI_MODEL_PROVIDER_TOKEN") && manifest.includes("NXQ_MALWARE_SCAN_ADAPTER_TOKEN") && manifest.includes("NXQ_RUNTIME_ENVIRONMENT")],
  ...
  ["Notification worker records truthful adapter health", notifications.includes("adapter_configured: adapterConfigured") && notifications.includes('target_worker_key: workerName') && notifications.includes('target_status: adapterConfigured ? "healthy" : "degraded"')],
