...
export const capabilityRules: CapabilityRule[] = [
  ...
  },
  {
    ...
    capabilityLevel: 2,
    keywords: ["lead form", "quote request", "estimate request", "request a quote", "get a quote", "application form", "job application"],
    ...
    requiresExternalApi: false,
    riskLevel: "low",
    clientSafeResponse: "NXQ can plan a lead capture or quote request flow. Owner review is required before confirming final scope.",
    ownerInternalNote: "Good launch feature. Confirm form fields, notification path, and where submissions should be stored.",
  },
  {
    featureName: "Booking request system",
    category: "booking",
    capabilityLevel: 2,
    keywords: ["booking", "appointment", "schedule", "calendar", "reservation", "book online", "schedule online"],
    ...
    requiresExternalApi: false,
    riskLevel: "medium",
    clientSafeResponse: "NXQ can plan a booking or appointment request system. Live calendar syncing may require additional setup and owner approval.",
    ownerInternalNote: "Start with booking request flow. Treat live calendar integrations as custom scope.",
  },
  {
    featureName: "Basic product catalog",
    category: "ecommerce",
    capabilityLevel: 2,
    keywords: ["product catalog", "products", "clothing", "shirts", "hoodies", "sell clothes", "store", "shop", "sizes", "colors"],
    decision: "owner_review_required",
    requiresOwnerApproval: true,
    requiresCustomQuote: false,
    requiresPaymentProvider: false,
    requiresExternalApi: false,
    riskLevel: "medium",
    clientSafeResponse: "NXQ can support a product catalog and order request flow. Full checkout/payment setup requires owner review.",
    ownerInternalNote: "Good launch target. Product catalog and manual order requests are safer than full checkout until payment provider setup is ready.",
  },
  {
