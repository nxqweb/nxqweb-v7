...
import { ArrowLeft, CheckCircle2, Save, ShoppingBag } from "lucide-react";
    ...
    <main className="nxq-page">
      <section className="portal-shell">
        <div className="panel-title panel-title-row">
          <div className="panel-title">
            ...
            <div>
              ...
              <p className="subtle">
                Tell NXQ how your store should work before products, checkout, and storefront pages are built.
              </p>
            </div>
          </div>
        ...
        <div className="notice-card">
          Saving or submitting this form does not publish a store, activate payments, or change your current plan.
        </div>
          ...
          <div className="owner-detail-grid">
            <section className="panel panel-wide">
              <div className="panel-title">
                ...
                <div>
                  <h2>Store basics</h2>
                  <p className="subtle">Start with what you sell and how the catalog should be organized.</p>
                </div>
              </div>
              ...
              <div className="setup-form-grid">
                <label>
                  ...
                  <input className="auth-input" value={intake.store_name} disabled={isLocked} onChange={(event) => updateField("store_name", event.target.value)} placeholder="Example: Hale Peak Supply" />
                </label>
                <label>
                  <span>Business model</span>
                  <select className="auth-input" value={intake.business_model} disabled={isLocked} onChange={(event) => updateField("business_model", event.target.value)}>
                    <option value="physical_products">Physical products</option>
                    <option value="digital_products">Digital products</option>
                    <option value="services">Services sold online</option>
                    <option value="mixed">Mixed products and services</option>
                  </select>
                </label>
                <label>
                  <span>Approximate product count</span>
                  <input className="auth-input" value={intake.product_count_range} disabled={isLocked} onChange={(event) => updateField("product_count_range", event.target.value)} placeholder="Example: 25-50 products" />
                </label>
                <label>
                  <span>Inventory method</span>
                  <select className="auth-input" value={intake.inventory_method} disabled={isLocked} onChange={(event) => updateField("inventory_method", event.target.value)}>
                    <option value="track_inventory">Track exact inventory</option>
                    <option value="made_to_order">Made to order</option>
                    <option value="unlimited">Unlimited inventory</option>
            ...
            <section className="panel panel-wide">
              ...
              <div className="setup-form-grid">
                {fulfillmentOptions.map(([value, label]) => (
                  <label className="settings-card" key={value}>
                    <span>{label}</span>
                    <input type="checkbox" checked={intake.fulfillment_methods.includes(value)} disabled={isLocked} onChange={() => toggleFulfillment(value)} />
                  </label>
                ))}
              </div>

              <div className="setup-form-grid">
                <label><span>Customer accounts</span><select className="auth-input" value={intake.customer_accounts_preference} disabled={isLocked} onChange={(event) => updateField("customer_accounts_preference", event.target.value)}><option value="disabled">Disabled</option><option value="optional">Optional</option><option value="required">Required</option><option value="not_sure">Not sure yet</option></select></label>
                <label className="settings-card"><span>Allow guest checkout</span><input type="checkbox" checked={intake.guest_checkout_preference} disabled={isLocked} onChange={(event) => updateField("guest_checkout_preference", event.target.checked)} /></label>
              </div>
            ...
            <section className="panel panel-wide">
              <div className="panel-title"><ShoppingBag size={20} /><div><h2>Policies, design, and migration</h2><p className="subtle">Give NXQ the remaining information needed to plan the storefront.</p></div></div>
              ...
              <label className="auth-label"><span>Current store URL</span><input className="auth-input" value={intake.current_store_url} disabled={isLocked} onChange={(event) => updateField("current_store_url", event.target.value)} placeholder="Leave blank when starting from scratch" /></label>
              ...
              <label className="auth-label"><span>Storefront style</span><textarea className="auth-input" rows={3} value={intake.storefront_style} disabled={isLocked} onChange={(event) => updateField("storefront_style", event.target.value)} /></label>
            ...
            <section className="panel panel-wide">
              ...
              <div className="setup-form-grid">
                <button className="icon-btn" type="button" disabled={saving || isLocked} onClick={() => void saveIntake(false)}><Save size={16} />{saving ? "Saving..." : "Save draft"}</button>
                <button className="wide-btn" type="button" disabled={saving || isLocked} onClick={() => void saveIntake(true)}><CheckCircle2 size={16} />{saving ? "Submitting..." : "Submit for NXQ review"}</button>
              </div>
