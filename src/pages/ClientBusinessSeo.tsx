...
export function ClientBusinessSeo(){
  ...
  useEffect(()=>{let active=true;async function load(){
    if(!isSupabaseConfigured||!supabase){if(active){setError("SEO status is temporarily unavailable.");setLoading(false);}return;}
    ...
    if(client.error||!client.data){if(active){setError("SEO status could not be loaded right now. No website changes were made.");setLoading(false);}return;}
    ...
    if(project.error||!project.data){if(active){setError("SEO status is not ready for this website yet.");setLoading(false);}return;}
    ...
    if(!active)return;if(issueRes.error||artifactRes.error||runRes.error){setError("SEO data could not be verified right now. Unverified issues and publish states are not shown.");setLoading(false);return;}
    setIssues((issueRes.data||[]) as SeoIssue[]);setArtifacts((artifactRes.data||[]) as SeoArtifact[]);setLatestRun((runRes.data||null) as SeoRun|null);setVerified(true);setLoading(false);
  ...
  return <main className="nxq-page"><section className="portal-shell">
    ...
    <section className="panel panel-wide"><h2>SEO maintenance publishing</h2><p className="subtle">SEO maintenance remains behind preview, verification, approval, and production safety gates.</p>{!latestRun?<div className="empty-state">No verified SEO maintenance publish run is recorded yet.</div>:<article className="owner-message-card"><div className="panel-title panel-title-row"><div><strong>{runLabel(latestRun.status)}</strong><p className="subtle">Started {new Date(latestRun.created_at).toLocaleString()} · protected maintenance path</p></div><span className="status-summary">{latestRun.status.replaceAll("_"," ")}</span></div>{latestRun.last_error?<div className="auth-error">This SEO maintenance run needs attention. Internal failure details are hidden from the client portal.</div>:null}{latestRun.preview_verified_at?<p className="subtle">Preview verified {new Date(latestRun.preview_verified_at).toLocaleString()}.</p>:null}{latestRun.published_at?<p className="subtle">Production verification recorded {new Date(latestRun.published_at).toLocaleString()}.</p>:null}{latestRun.production_url?<a className="icon-btn" href={latestRun.production_url} target="_blank" rel="noopener noreferrer"><ExternalLink size={16}/>View live website</a>:null}</article>}</section>
    <section className="panel panel-wide"><h2>SEO issues</h2>{open.length===0?<div className="empty-state"><CheckCircle2 size={18}/> No unresolved SEO issues are recorded.</div>:open.map((issue)=><article className="owner-message-card" key={issue.id}><div className="panel-title panel-title-row"><div><strong>{issue.title}</strong><p className="subtle">{issue.category} · {issue.severity} · {issue.status.replaceAll("_"," ")}</p></div>{issue.severity==="high"||issue.severity==="critical"?<ShieldAlert size={18}/>:null}</div>{issue.summary?<p>{issue.summary}</p>:null}<small>{issue.auto_fixable?"This issue may qualify for the guarded safe-fix path after required checks.":"This issue may require review or external evidence before changes are made."}</small></article>)}</section>
    <section className="panel panel-wide"><h2>Sitemap & schema artifacts</h2>{artifacts.length===0?<div className="empty-state">No verified SEO artifacts are recorded yet.</div>:artifacts.map((artifact)=><article className="owner-message-card" key={artifact.id}><strong>{artifact.artifact_type.replaceAll("_"," ")}</strong><p className="subtle">{artifact.status}</p>{artifact.last_error?<div className="auth-error">This artifact needs attention. Internal failure details are hidden from the client portal.</div>:null}</article>)}</section></>:null}
  </section></main>;
}
