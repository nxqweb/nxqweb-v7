-- Fix maintenance bootstrap after a verified production deployment.
-- project_deployments stores its published URL in deploy_url; published_url
-- belongs to production_launch_requests.
...
begin
  for project_row in
    ...
    left join lateral (
      select coalesce(plc.published_url, pd.deploy_url, plc.production_url) as monitored_url
