-- Wave 13: deterministic Edge routing only for structured patches the change worker can actually apply.
-- Unstructured/ambiguous requests go to classification instead of wasting retries in the wrong worker.
...
begin
  ...
  structured_safe := jsonb_typeof(patch)='object' and patch ?| array[
    'contact_phone','contact_email','service_area','goals','desired_style','about','add_services','remove_services'
  ];
...
comment on function public.route_submitted_change_request() is 'Routes only supported low-risk structured patches to deterministic Edge automation; all other requests go to classification, preserving backend-first automation without wasteful retries.';
