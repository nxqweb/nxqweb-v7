...
create table if not exists public.commerce_request_reference_upload_tickets (
  request_id uuid primary key references public.commerce_customer_requests(id) on delete cascade,
  client_id uuid not null references public.clients(id) on delete cascade,
  token_hash text not null,
  expected_file_count integer not null check (expected_file_count between 1 and 10),
  uploaded_file_count integer not null default 0 check (uploaded_file_count between 0 and 10),
  max_file_size_bytes bigint not null check (max_file_size_bytes between 1 and 20971520),
  expires_at timestamptz not null,
  revoked_at timestamptz,
  created_at timestamptz not null default now()
);

create table if not exists public.commerce_customer_request_reference_files (
  request_id uuid not null references public.commerce_customer_requests(id) on delete cascade,
  client_file_id uuid primary key references public.client_files(id) on delete cascade,
  client_id uuid not null references public.clients(id) on delete cascade,
  customer_file_name text not null,
  created_at timestamptz not null default now()
);

create index if not exists commerce_request_reference_files_request_idx
  on public.commerce_customer_request_reference_files (request_id, created_at);
create index if not exists commerce_request_reference_files_client_idx
  on public.commerce_customer_request_reference_files (client_id, created_at desc);

alter table public.commerce_request_reference_upload_tickets enable row level security;
alter table public.commerce_customer_request_reference_files enable row level security;

revoke all on public.commerce_request_reference_upload_tickets from public, anon, authenticated;
revoke all on public.commerce_customer_request_reference_files from public, anon, authenticated;
grant select, insert, update, delete on public.commerce_request_reference_upload_tickets to service_role;
grant select, insert, update, delete on public.commerce_customer_request_reference_files to service_role;
grant select on public.commerce_customer_request_reference_files to authenticated;

create policy client_view_own_commerce_request_reference_files
on public.commerce_customer_request_reference_files for select to authenticated
using (exists (
  select 1 from public.clients c
  where c.id = commerce_customer_request_reference_files.client_id
    and c.auth_user_id = auth.uid()
));

create policy owner_view_commerce_request_reference_files
on public.commerce_customer_request_reference_files for select to authenticated
...
begin
  ...
  if v_upload_count > 0 then
    ...
    insert into public.commerce_request_reference_upload_tickets (
      ...
      v_row.id, v_row.client_id, encode(digest(v_upload_ticket, 'sha256'), 'hex'),
      ...
      to_jsonb(r) || jsonb_build_object(
        'reference_images', coalesce((
          select jsonb_agg(jsonb_build_object(
            'client_file_id', rf.client_file_id,
            'file_name', rf.customer_file_name,
            'scan_status', coalesce(s.status, 'pending'),
            'quarantine_status', coalesce(s.quarantine_status, 'restricted')
          ) order by rf.created_at)
          from public.commerce_customer_request_reference_files rf
          left join public.client_file_security_scans s
            on s.client_file_id = rf.client_file_id and s.client_id = rf.client_id
          where rf.request_id = r.id and rf.client_id = v_client_id
        ), '[]'::jsonb)
...
revoke all on function public.resolve_commerce_request_reference_upload(uuid, text) from public, anon, authenticated, service_role;
revoke all on function public.register_commerce_request_reference_upload(uuid, text, text, text, text, bigint) from public, anon, authenticated, service_role;
...
comment on table public.commerce_request_reference_upload_tickets is
  'Hashed, short-lived, request-scoped capabilities for bounded Commerce reference-image uploads.';
comment on table public.commerce_customer_request_reference_files is
  'Tenant-bound relation between Commerce requests and private client files; access remains scan-gated.';
comment on function public.register_commerce_request_reference_upload(uuid, text, text, text, text, bigint) is
  'Service-only atomic registration of an existing request-scoped private object; the client_files trigger queues restricted scanning.';
