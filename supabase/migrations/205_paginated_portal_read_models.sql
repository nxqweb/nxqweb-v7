-- Cursor-paginated portal read models.
-- Prevents browser-side all-row loading as NXQ client/history counts grow.

create index if not exists clients_created_id_desc_idx
...
create index if not exists client_messages_client_created_id_desc_idx
...
comment on function public.owner_client_list_page(integer, timestamptz, uuid, text, text) is
  'Owner-only cursor-paginated client list. Keeps portal list payloads bounded as tenant count grows.';
comment on function public.current_client_message_page(integer, timestamptz, uuid) is
  'Tenant-derived cursor-paginated message history. Client identity is never accepted from the caller.';
