-- Atomically approve or deny client plan-change requests.
...
begin
  ...
  if clean_decision = 'accepted' then
    ...
    set product_family_id = request_row.requested_product_family_id,
        ...
        monthly_price = coalesce(request_row.requested_monthly_price, monthly_price),
        updated_at = now()
    ...
    insert into public.activity_logs (
      ...
      jsonb_build_object(
        ...
        'monthly_price', request_row.requested_monthly_price,
...
comment on function public.resolve_client_plan_change(uuid, text, text, numeric) is
  'Owner-only atomic resolver for client plan-change requests. Applies family, tier, and monthly price only after explicit approval.';
