-- Surface externally-blocked but safely deferred jobs in the owner exception center.
-- These jobs remain queued for automatic retry and are warnings, not exhausted NXQ failures.
...
comment on function public.owner_exception_center() is
  'Owner-only operational read model including safely deferred external provider billing/credit blockers.';
