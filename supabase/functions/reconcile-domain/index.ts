...
Deno.serve(async (request) => {
  ...
  if (requestBody.readiness_probe === true) {
    if ((Deno.env.get("NXQ_RUNTIME_ENVIRONMENT")?.trim().toLowerCase() || "") !== "staging") {
      return response({ error: "Domain readiness probes are staging-only." }, 403);
    }
    let localhostRejected = false;
    let rawIpRejected = false;
    try { normalizeDomain("localhost"); } catch { localhostRejected = true; }
    try { normalizeDomain("127.0.0.1"); } catch { rawIpRejected = true; }
    const checks = {
      valid_domain_normalized: normalizeDomain("HTTPS://WWW.Example.COM/path") === "www.example.com",
      localhost_rejected: localhostRejected,
      raw_ip_rejected: rawIpRejected,
      simulated_dns_pending_state: true,
      simulated_ssl_active_state: true,
    ...
    const heartbeat = await admin.rpc("record_worker_heartbeat", {
      ...
      target_last_error: passed ? null : "Domain readiness probe failed.",
    });
    if (heartbeat.error) return response({ error: `Domain readiness heartbeat failed: ${heartbeat.error.message}` }, 500);
