...
function asAdapterResponse(value: unknown): AdapterResponse {
...
async function postAdapter(delivery: Delivery) {
  ...
  try {
    ...
    let body: AdapterResponse = {};
    try {
      body = text ? asAdapterResponse(JSON.parse(text) as unknown) : {};
    } catch {
      body = { message: text };
    }
    ...
    return {
      provider_message_id: String(body.provider_message_id || body.id || ""),
      provider_status: String(body.status || "delivered"),
    };
  } finally {
  ...
  try {
    ...
    for (const raw of due.data || []) {
      ...
      const claim = await admin
        .from("notification_deliveries")
        .update({ status: "sending", attempts: Number(delivery.attempts || 0) + 1, updated_at: new Date().toISOString() })
        .eq("id", delivery.id)
        .in("status", ["queued", "failed"])
        .select("*")
        .maybeSingle();
      ...
      try {
        ...
        const endpointReady = Boolean(
          Deno.env.get("NXQ_NOTIFICATION_ADAPTER_URL")?.trim() && Deno.env.get("NXQ_NOTIFICATION_ADAPTER_TOKEN")?.trim(),
        );
        if (!endpointReady) {
          await admin
            .from("notification_deliveries")
            .update({ status: "blocked", last_error: "Notification provider adapter is not configured.", updated_at: new Date().toISOString() })
            .eq("id", current.id);
          blocked++;
          continue;
        }
        ...
        await admin
          ...
          .update({
            ...
            metadata: { ...(current.metadata || {}), provider_status: result.provider_status },
          })
          .eq("id", current.id);
        delivered++;
      } catch (error) {
        ...
        await admin
          ...
          .update({
            ...
            run_after: exhausted
              ? current.metadata?.run_after
              : new Date(Date.now() + Math.min(3600000, Math.max(120000, 2 ** Math.min(current.attempts, 5) * 60000))).toISOString(),
            updated_at: new Date().toISOString(),
          })
          .eq("id", current.id);

        if (exhausted && current.client_id) {
          await admin.from("automation_escalations").insert({
            client_id: current.client_id,
            project_id: current.project_id,
            escalation_type: "notification_delivery_exhausted",
            severity: "warning",
            title: "Notification delivery needs attention",
            summary: `A ${current.channel} notification exhausted automatic retries.`,
            details: { notification_delivery_id: current.id, channel: current.channel, error: message },
          });
        }
        failed++;
      }
    ...
    return response({ ok: false, error: error instanceof Error ? error.message : "Notification dispatcher failed." }, 500);
  }
});
