...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    if(!authorization.toLowerCase().startsWith("bearer "))return response({ok:false,error:"Authentication required."},401);
    ...
    if(owner.error||!owner.data?.id)return response({ok:false,error:"Owner access required."},403);
    ...
    if(!/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(clientFileId))return response({ok:false,error:"A valid client file id is required."},400);
    ...
    if(file.data.expires_at&&new Date(String(file.data.expires_at)).getTime()<=Date.now())return response({ok:false,error:"File access has expired."},410);
    ...
    if(scan.error||!scan.data)return response({ok:false,error:"File security verification is not complete."},423);
    if(scan.data.status!=="clean"||scan.data.quarantine_status!=="released"||!scan.data.released_at)return response({ok:false,error:"File remains restricted by NXQ file security."},423);

    const bucket=String(file.data.bucket_id||"client-files").trim();
    const storagePath=String(file.data.storage_path||"").trim();
    if(!bucket||!storagePath||storagePath.includes("..")||storagePath.startsWith("/"))return response({ok:false,error:"Stored file reference is invalid."},500);
    ...
    if(signed.error||!signed.data?.signedUrl)return response({ok:false,error:"Unable to create temporary file access."},500);
    ...
    return response({ok:true,signed_url:signed.data.signedUrl,expires_in_seconds:60,file_name:file.data.file_name});
  }catch(error){return response({ok:false,error:error instanceof Error?error.message:"Secure owner file access failed."},500);}
});
